...
void xq_handler::operator()(const request& req, reply& rep)
{
  // Decode url to path.
...
#endif
  rep.content.append("<html><body>hello, world!</body></html>");
...
bool xq_handler::url_decode(const std::string& in, std::string& out)
{
  out.clear();
  out.reserve(in.size());
