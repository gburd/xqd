...
AC_DEFUN(AC_ACME_TM_GMTOFF,
    ...
    AC_CACHE_VAL(ac_cv_xdbms_tm_has_tm_gmtoff,
	AC_TRY_COMPILE([
#	include <sys/types.h>
#	include <time.h>],
	[u_int i = sizeof(((struct tm *)0)->tm_gmtoff)],
	ac_cv_xdbms_tm_has_tm_gmtoff=yes,
	ac_cv_xdbms_tm_has_tm_gmtoff=no))
    AC_MSG_RESULT($ac_cv_xdbms_tm_has_tm_gmtoff)
    if test $ac_cv_xdbms_tm_has_tm_gmtoff = yes ; then
...
AC_DEFUN(AC_ACME_INT64T,
    ...
    AC_CACHE_VAL(ac_cv_xdbms_int64_t,
	AC_TRY_COMPILE([
#	include <sys/types.h>],
	[int64_t i64],
	ac_cv_xdbms_int64_t=yes,
	ac_cv_xdbms_int64_t=no))
    AC_MSG_RESULT($ac_cv_xdbms_int64_t)
    if test $ac_cv_xdbms_int64_t = yes ; then
...
AC_DEFUN(AC_ACME_SOCKLENT,
    ...
    AC_CACHE_VAL(ac_cv_xdbms_socklen_t,
	AC_TRY_COMPILE([
#	include <sys/types.h>
#	include <sys/socket.h>],
	[socklen_t slen],
	ac_cv_xdbms_socklen_t=yes,
	ac_cv_xdbms_socklen_t=no))
    AC_MSG_RESULT($ac_cv_xdbms_socklen_t)
    if test $ac_cv_xdbms_socklen_t = yes ; then
